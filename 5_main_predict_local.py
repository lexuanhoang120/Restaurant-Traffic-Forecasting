...
def process_data_prediction_time(df: pd.DataFrame, year_month: str) -> pd.DataFrame:
    ...
    filtered_data_1.loc[df['shift_date'].dt.strftime('%Y-%m') == f'{year}-{month}', 'guest_count'] = None
...
if __name__ == "__main__":
    ...
    print("{:<30}{}".format("path_cluster_km:", path_cluster_km))
    ...
    for time in list_prediction:
        ...
        if path_feature_data is None:
            ...
            data_feature = Basic_Features.create_features_global_2(dataset)
            ...
            path_dataset_time = path_dataset + f"local-{time}.parquet"
        ...
        file_train_time = path_train_set + f"train-local-{time}.parquet"
        ...
        file_test_time = path_test_set + f"test-local-{time}.parquet"
        ...
        reg = xgb.XGBRegressor(n_estimators=500, learning_rate=0.02, max_depth=6, verbosity=0, silent=True,
                               random_state=42, min_child_weight=1, gamma=0.6, subsample=0.8, colsample_bytree=0.8,
                               n_jobs=-1)
    ...
    # Save prediction
    df_final_test.reset_index().to_parquet(path_local_result, index=False)
    print("{:<80}{}".format(f"Save prediction to {path_local_result}", "SUCCESS"))
